...
function order_goods($order_id)
{
    ...
    foreach ($res as $row) {
        ...
        if ($row['extension_code'] == "presale" && !empty($extension_id)) {
            $row['url'] = build_uri('presale', ['presaleid' => $extension_id], $row['goods_name']);
        ...
        $row['shopUrl'] = url('store/index/index', ['id' => $row['ru_id']]);
        $goods_list[] = $row;
    }
...
{
    $rec_txt = ['普通', '团够', '拍卖', '夺宝奇兵', '积分商城', '预售', '秒杀'];
    ...
    foreach ($arr as $key => $value) {
        ...
        if ($value['extension_code'] == 'package_buy') {
            ...
            if ($value['rec_type'] == 1) {
                ...
                $arr[$key]['url'] = build_uri('group_buy', ['gbid' => $group_buy['act_id']]);
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = [], $parent = 0, $warehouse_id = 0, $area_id = 0, $store_id = 0, $take_time = '', $store_mobile = '')
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    ...
    if ($num > 0) {
        ...
        $sql = "SELECT goods_number FROM " . $GLOBALS['ecs']->table('cart') .
            ...
            " AND extension_code <> 'package_buy' " .
            " AND rec_type = '" . CART_GENERAL_GOODS . "' AND group_id='' AND warehouse_id = '$warehouse_id' AND stages_qishu='-1' AND store_id = '" . $store_id . "' "; //by mike add
        ...
        if ($row) { //如果购物车已经有此物品，则更新
            ...
            if ($GLOBALS['_CFG']['use_storage'] == 0 || $num <= $goods_storage) {
                ...
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number = '$num'" .
                    ...
                    "AND rec_type = '" . CART_GENERAL_GOODS . "' AND group_id = 0 AND stages_qishu='-1' AND store_id = " . $store_id;
 ...
 * @return  boolean
 */
function addto_cart_combo($goods_id, $num = 1, $spec = [], $parent = 0, $group = '', $warehouse_id = 0, $area_id = 0, $goods_attr = '')
...
function get_insert_group_main($goods_id, $num = 1, $goods_spec = [], $parent = 0, $group = '', $warehouse_id = 0, $area_id = 0)
{
    $ok_arr['is_ok'] = 0;
    $spec = $goods_spec;
 ...
 * @return  array
 */
function get_cart_goods($cart_value = '', $type = 0, $warehouse_id = 0, $area_id = 0, $favourable_list = [])
{
    $goods_where = " AND g.is_delete = 0 ";
    if ($type == CART_PRESALE_GOODS) {
        $goods_where .= " AND g.is_on_sale = 0 ";
    ...
    return ['goods_list' => $goods_list_dsh, 'total' => $total, 'goods_list_old' => $goods_list];
}
...
function get_cart_ru_goods_list($goods_list, $cart_value = '', $consignee = '', $store_id = 0)
{
    ...
    foreach ($goods_list as $key => $row) {
        ...
        $arr[$key]['url'] = build_uri('merchants_store', ['urid' => $key], $ru_name);
...
function get_ru_shippng_info($cart_goods, $cart_value, $ru_id, $consignee = '')
{
    ...
    $arr = ['is_freight' => $is_freight, 'shipping_list' => $shipping_list];
    return $arr;
}
 ...
 */
function compute_discount($type = 0, $newInfo = [], $use_type = 0, $ru_id = 0)
{
    //ecmoban模板堂 --zhuo start
    if (!empty($_SESSION['user_id'])) {
        $c_sess = " c.user_id = '" . $_SESSION['user_id'] . "' ";
