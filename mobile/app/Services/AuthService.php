...
class AuthService
{
     ...
     * @return
     */
    public function loginMiddleSilent(array $request)
    {
        $result = $this->wxSilentLogin($request);
     ...
     * @return
     */
    private function wxSilentLogin($req){
        $config = [
        ...
        $response['unionid'] = empty($response['unionid']) ? $response['openid'] : $response['unionid'];
...
//        $this->wechatUserUpdate($args);
     ...
     * @return
     */
    public function loginMiddleSellerSilent(array $request)
    {
        $result = $this->wxSellerSilentLogin($request);
     ...
     * @return
     */
    private function wxSellerSilentLogin($req)
    {
        $token = $req['sellertoken'];
        $code = $req['code'];
        $res = DB::select("select * from dsc_wxapp where token =  '$token'  ");
        $config = [
            'appid' => $res[0]->appid,
            'secret' => $res[0]->appsecret,
        ];
        $wxapp = new Wxapp($config);
        ...
        return ['token' => $token, 'openid' => $response['openid']];
    }
    ...
    public function getWxUserInfo($uid, $openid)
    {
        $re = $this->WechatUserRepository->getWxUserInfo($uid);
        if ($re) {
            print_r($re);
            die;
            return $re;
