...
//            'notify_url' => Url::to('api/wx/payment/notify', ['code', $code]),
            'notify_url' => Url::to('api/wx/payment/asyncnotify'),
    ...
    public function asyncNotify($obj)
    {
        $wxpay = new WxPay();
        ...
        $wxpay->init($config['app_id'], $config['app_secret'], $config['mch_key']);
        $wxOrder = json_decode(json_encode($obj), true);
        $tmpArr = $wxOrder;
        unset($tmpArr['sign']);
        ksort($tmpArr);
        $str = '';
        ...
        if ($wxOrder['sign'] == strtoupper(md5($str))) {
            $order = $this->orderRepository->orderPayByOutTradeNo($obj->out_trade_no);
            $uid = $order['user_id'];
            ...
            return $wxpay->getSucessXml();
        } else {
            return $wxpay->getFailXml();
        }
    }
}
