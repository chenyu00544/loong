...
//            'country' => !empty( $args['country'] ) ? $args['country'] : '',
    ...
    {
        $address = $this->addressRepository->getDefaultByUserId($userId);
        if (empty($address['province']) || empty($address['city'])) {
            $result['default_address'] = '';
        } else {
            $result['default_address'] = [
                'country'    => $this->regionRepository->getRegionName($address['country']),
                'province'   => $this->regionRepository->getRegionName($address['province']),
                'city'       => $this->regionRepository->getRegionName($address['city']),
                'district'   => $this->regionRepository->getRegionName($address['district']),
