...
class BargainService
{
    ...
    public function bargainGoodsList($page = 1, $size = 10, $user_id = 0)
    {
        ...
        foreach ($goodsList as $key => $val){
            $goodsList[$key]['end_time'] = $val['end_time'] + 8*3600;
            $goodsList[$key]['start_time'] = $val['start_time'] + 8*3600;
        }
        return $goodsList;
    }
    ...
    public function goBargain($bargain_id = 0, $bs_id = 0, $user_id= 0, $form_id = '')
    {
        ...
        if($bargain['target_price'] == $bs_log['final_price']){            
            ...
            $subtract = $this->bargainRepository->subtractPriceSum($bs_id);
            if($subtract == 0){
                $bargainPrice = $bargain['goods_price'] - $bargain['target_price'];
                $subtract_price = rand($bargainPrice*0.2, $bargainPrice*0.4);//砍掉价格区间
            }else{
        ...
        if($add){
			...
			//验证是否参与当前活动
			$add_bargain = $this->bargainRepository->isAddBargain($bargain_id, $user_id);
			if($add_bargain){
				//推送模消息
				/**/
				$pushData = [
