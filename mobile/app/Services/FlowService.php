...
class FlowService
{
     ...
     * @return array
     */
    public function flowInfo($userId, $flow_type = 0, $bs_id = 0, $t_id = 0, $team_id = 0, $p_id=[])
    {
        $result = [];
        $arrru = [];
        $arrbo = [];
        ...
        $result['cart_goods_list'] = $this->arrangeCartGoods($userId, $flow_type, $p_id);  //购物车商品
        foreach ($result['cart_goods_list']['list'] as $key => $val) {
            ...
            $bonus[$key] = $this->bonusRepository->UserBonus($userId, true, $result['cart_goods_list']['order_total'], $val, true);  //用户红包
        }
        $cou_num = count($cou);
        if ($cou_num > 0) {
            for ($i = 0; $i < $cou_num; $i++) {
        ...
        $bonus_num = count($bonus);
        if ($bonus_num > 0) {
            for ($i = 0; $i < $bonus_num; $i++) {
                $arrbo = array_merge($arrbo, $bonus[$i]);
            }
            $result['bonus_list'] = $arrbo;
        }
...
//        $result['payment_list'] = $this->paymentRepository->paymentList();   //支付方式列表
    ...
    /////整个购物车
    public function get_user_pintuan_coupons_list_cart($user_id = '', $cartGoods, $num = 10)
    {
        $time = gmtime();
        ...
        foreach ($cartGoods as $val) {
            foreach ($val['goods'] as $v) {
                ...
                $sql = " SELECT c.*,cu.*,o.order_sn,o.add_time FROM dsc_coupons_user cu LEFT JOIN dsc_coupons c ON c.cou_id=cu.cou_id LEFT JOIN dsc_order_info o ON cu.order_id=o.order_id $where ";
     ...
     * @return array
     */
    private function arrangeCartGoods($userId, $flow_type, $p_id= [])
    {
        $cartGoodsList = $this->cartRepository->getGoodsInCartByUser($userId, $flow_type, [], $p_id);  //购物车商品
        $list = [];
...
//        sort($list);
        ...
        return ['list' => $slist, 'order_total' => $totalAmount, 'order_total_formated' => price_format($totalAmount, false)];
    }
    ...
    public function submitOrder($args)
    {
        ...
        if(isset($args['product_ids'])){
            $product_ids = explode(',', $args['product_ids']);
        }else{
            $product_ids = [];
        }
        ...
        // 检查购物车商品
        $goodsNum = $this->cartRepository->goodsNumInCartByUser($userId, $flow_type, $product_ids);
        ...
        if ($this->shopConfigRepository->getShopConfigByCode('use_storage') == 1 && $this->shopConfigRepository->getShopConfigByCode('stock_dec_time') == 1) {
            $cart_goods = $this->cartRepository->getGoodsInCartByUser($userId, $flow_type, $product_ids);
            $_cart_goods_stock = [];
        ...
        if ($args['uc_id'] > 0) {
            ...
            ////////////////////////////////////////////////////////////////免单券
            if ($coupons['cou_type'] == 95 || $coupons['cou_type'] == 96) {
            ...
            if ($flow_type == CART_TEAM_GOODS) {
                if ($args['team_id'] < 1)    // > 0 说明是开团    新团才能用免单券。
                {

                    $coupGoodsList = $this->get_user_pintuan_coupons_list_cart($userId, $cartGoods);

                    if (!empty($coupGoodsList)) {
                        $has_free_coupons = true;

                        //$uc_id = $coupGoodsList[0]['uc_id'];
                        $uc_id = $coupGoodsList[0]->uc_id;
        ...
        /** 获取该订单中使用的红包cyc */
        if ($args['bn_id'] > 0) {
            $bonus = $result = $this->bonusRepository->getBonus($userId, $args['bn_id']);
            $amount = $total['amount'] - $bonus['type_money'];
            $total['amount'] = $amount > 0 ? $amount : '0.01';
            $total['goods_price'] = $total['goods_price'] - $bonus['type_money'];
        }
        /** end-cyc */
        ...
        if ($hasPtFree) {
            ...
            //$ptOrderId = $order['order_id'];

            $sql = "update dsc_coupons_user set is_use = 1,is_use_time = $useTime,order_id='$new_order_id'  WHERE uc_id =" . $ptUcId;
        ...
        /** 处理红包cyc*/
        if ($args['bn_id']) {
            $bntype = $this->bonusRepository->setupBonus($userId, $args['bn_id'], $order['order_id']);
        }
        /*end-cyc*/
...
//            $res = $GLOBALS['db']->getRow($sql);
    ...
    //cyc添加
    public function cashOnDelivery($order_id, $gzid = 0)
    {
        $order = [
            'pay_id' => 16,
            'pay_name' => '货到付款'
        ];
        $re = $this->orderRepository->cashOn($order, $order_id);
        $time = time();
        if ($gzid) {
            DB::update("insert into dsc_com_ads_order (gzid, order_id, add_time) values ('$gzid', '$order_id', '$time')");
        }
        return $re;
    }
     ...
     * @return array
     */
    public function changeBonus($bn_id, $userId, $flow_type = 0)
    {
        /* 获得收货人信息 */
        $this->defaultAddress = $defaultAddress = $this->addressRepository->getDefaultByUserId($userId);
        /* 对商品信息赋值 */
        ...
        if (empty($cart_goods_list)) {
            ...
            /* 获取红包信息 */
            $result = $this->bonusRepository->getBonus($userId, $bn_id);
        }
        return $result;
    }
}
