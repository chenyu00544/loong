...
class CartRepository implements CartRepositoryInterface
{
     ...
     * @return mixed
     */
    public function goodsNumInCartByUser($id, $flow_type = 0, $product_ids = [])
    {
        ...
        if (!empty($product_ids) && $product_ids[0] != '') {
            $cart->whereIn('rec_id', $product_ids);
        }
     ...
     * @return mixed
     */
    public function getGoodsInCartByUser($id, $flow_type = 0, $product_ids = [], $goods_ids = [])
    {
        $cart = Cart::select('cart.*')
            ->with(['goods' => function ($query) {
        ...
        if (!empty($goods_ids) && $goods_ids[0] != '') {
            $cart->whereIn('goods_id', $goods_ids);
            $cart->limit(1);
        }
        $cart_list = $cart->where('rec_type', $flow_type)//普通商品
        ->orderby('rec_id', 'desc')
            ->get()
            ->toArray();
        $cart = array();
