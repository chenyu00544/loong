...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    public function getOrderByUserId($id, $status = 0,$type = '', $page=0, $size=10)
    {
        ...
        $order = $model
            ->select(['order_id', 'order_sn', 'order_status', 'shipping_status', 'pay_status','goods_amount','order_amount','add_time','shipping_status','shipping_status', 'money_paid', 'shipping_fee', 'extension_code'])
     ...
     * @return mixed
     */
    public function cashOn($order, $orderId)
    {
        $re = OrderInfo::where('main_order_id', $orderId)
            ->update($order);
     ...
     * 根据微信订单号获取订单
     */
    public function orderPayByOutTradeNo($outTradeNo)
    {
        $order = OrderInfo::where('order_sn', $outTradeNo)
            ->first();
        return $order;
    }
