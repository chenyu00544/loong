...
use App\Contracts\Repositories\Bargain\BargainRepositoryInterface;
 ...
 */
class BargainRepository implements BargainRepositoryInterface
{
    protected $goods;
    private $field;
    private $authService;
    ...
    {
        $tatistics_og = BargainStatisticsLog::select('*')
            ->where('bargain_id', $bargain_id)
    ...
    function bargainTargetPrice($bargain_id = 0,$goods_id = 0,$attr_id = [], $warehouse_id = 0, $area_id = 0,$model_attr = 0)
    {
        ...
        if ($this->shopConfigRepository->getShopConfigByCode('goods_attr_price') == 1) {
            if ($model_attr == 1) {
                ...
                ->first();
                if($product_price){
                    $product_price = $product_price->toArray();
                }
            }
            $res =[];

            if(!empty($product_price['product_id'])){
