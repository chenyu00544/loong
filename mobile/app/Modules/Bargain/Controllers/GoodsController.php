...
class GoodsController extends FrontendController
{
    ...
    public function __construct()
    {
        ...
        $this->keywords = I('request.keywords', '', ['htmlspecialchars', 'trim']);
    ...
    public function actionIndex()
    {
        ...
        if (!empty($this->bs_id)) {
            ...
            $this->assign('bargain_num', $bargain_num); //参与砍价人数
            ...
            if ($bargain_log['goods_attr_id']) {
                ...
                $goods['shop_price'] = bargain_get_final_price($goods['goods_id'], 1, true, $spec, $warehouse_id, $area_id);//原价
                $goods['target_price'] = bargain_target_price($this->bargain_id, $goods['goods_id'], $spec);//底价
            }
            //进度条
            $surplus = $goods['shop_price'] - $goods['target_price'];//差价
            //已砍价
        ...
        $share_data = [
            ...
            'desc' => $goods['bargain_desc'] ? $goods['bargain_desc'] : $goods['goods_brief'],
            'link' => '',
            'img' => $goods['goods_img'],
        ];
    ...
    public function actionAddbargain()
    {
        ...
        //商品信息
        $bargain_goods = get_bargain_goods_info($bargain_id);

        //获取砍价商品属性价格
        if ($specs) {
            //$goods_price = bargain_goods_price($bargain_id,$goods_id,$spec);//原价
            $goods_price = bargain_get_final_price($goods_id, $number, true, $spec, $warehouse_id, $area_id);
        } else {
    ...
    public function actionIsSubscribe()
    {
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {//微信浏览器
            ...
            if ($userInfo['subscribe'] === 1) {
                $this->ajaxReturn(array('state' => 1, 'msg' => '已关注'));
            }else{
                //获取二维码
                $ticket = $this->weObj->getQRCode('bargain#'.$id, 3, 30);
                if($ticket){
                    $uri = $this->weObj->getQRUrl($ticket['ticket']);
                    $this->ajaxReturn(array('state' => 2, 'msg' => $uri));
                }
            }
        }else{
            $this->ajaxReturn(array('state' => 0, 'msg' => '请在微信打开'));
        }
    }
    //----cyc-end
    ...
    public function actionGobargain()
    {
        ...
        $bs_id = I('bs_id', 0, 'intval');//发起活动id
        $bargain_id = I('id', 0, 'intval');//活动id
        ...
        //活动砍价信息
        $bargain = $this->db->table('bargain_goods')->field('target_price,min_price,max_price')->where(['id' => $bargain_id])->find();
        //参与活动记录
        $bs_log = $this->db->table('bargain_statistics_log')->field('goods_attr_id,user_id,final_price,count_num')->where(['id' => $bs_id])->find();
        //获取选中活动属性原价，底价
        ...
        if ($bargain['target_price'] == $bs_log['final_price']) {
            $goods = $this->db->table('goods')->field('goods_name')->where(['goods_id' => $goods_id])->find();
            //推送模消息
            $pushData = [
                'keyword1' => ['value' => $goods['goods_name'], 'color' => '#173177'],     //商品名称
                'keyword2' => ['value' => $bargain['target_price'], 'color' => '#173177']  //底价
            ];
            $url = __HOST__ . url('bargain/goods/index', ['id' => $bargain_id]);
            push_template('OPENTM410292733', $pushData, $url, $bs_log['user_id']);
    ...
    public function actionBargainprice()
    {
        ...
        if ($bargain_id == 0) {
            ...
            //限制用户购买的数量 bywanglu
            if ($attr_id) {
                $shop_price = bargain_get_final_price($goods['goods_id'], $number, true, $attr_id, $warehouse_id, $area_id);
                //ecmoban模板堂 --zhuo end
                ...
                //属性价格
                $spec_price = bargain_get_final_price($goods['goods_id'], $number, true, $attr_id, $warehouse_id, $area_id, 1, 0, 0, $res['show_goods']);
