...
class AdminController extends BackendController
{
    ...
    public function actionIndex()
    {
        ...
        $goods_name = I('post.keyword', '', ['htmlspecialchars', 'trim']);
    ...
    public function actionAddgoods()
    {
        ...
        if (IS_POST) {
            ...
            if (!$id) {//添加
                $count = $this->model->table('bargain_goods')->where(['goods_id' => $data['goods_id'], 'status' => '0'])->count();
                if ($count >= 1) {
                    exit(json_encode(['status' => 'n', 'info' => '此砍价商品活动结束之前，不可添加新的活动']));
                }
    ...
    public function actionEditgoods()
    {
        ...
        $sort_order = empty($_REQUEST['sort_order']) ? 0 : intval($_REQUEST['sort_order']);
        $result = [
            'error' => 0,
            'message' => '',
            'content' => '修改失败'
        ];
        $sql = "SELECT $type FROM {pre}bargain_goods WHERE id = $id";
        $res = $GLOBALS['db']->getOne($sql);

        if ($res == 1) {
            $sql = "UPDATE {pre}bargain_goods SET $type = 0 WHERE id = $id ";
        } else {
            $sql = "UPDATE {pre}bargain_goods SET $type = 1 WHERE id = $id ";
        }
        if ($type == 'sort_order') {
            $sql = "UPDATE {pre}bargain_goods SET sort_order = $sort_order WHERE id = $id ";
        }
