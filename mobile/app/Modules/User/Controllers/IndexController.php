...
class IndexController extends FrontendController
{
    ...
    public function actionHistory()
    {
        ...
        if (IS_AJAX) {
            ...
            die(json_encode(['history' => $history['goods_list'], 'totalPage' => $page_size, 'count' => $count]));
        }
    ...
    public function actionAddAddress()
    {
        if (IS_POST) {
            ...
            if (empty($consignee['mobile'])) {
                exit(json_encode(['status' => 'n', 'info' => L('msg_input_mobile')]));
            }
            // 验证手机号格式
            ...
            if (empty($consignee['province']) || empty($consignee['city']) || empty($consignee['district'])) {
                exit(json_encode(['status' => 'n', 'info' => L('select_Local_area')]));
            }
            // 收货地址数量限制
   ...
   */
    ...
    public function actionImgReturn()
    {
        ...
        //上传图片并 获得路径
        $result = $this->upload('data/report_img', false, 2, [600, 600]);
        $path = $result['url']['myfile']['url'];
        $add_time = gmtime();
