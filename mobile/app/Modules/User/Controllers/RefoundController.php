...
class RefoundController extends FrontendController
{
    ...
    {
        $order_id = I('order_id', 0, ['htmlspecialchars','trim']);
        $page = I('page', 1);
        $size = I('size', 1);
        $type = I('type', 0);
        ...
        if (IS_AJAX) {
            if ($type == 0) {
                ...
                die(json_encode(['order_list' => $order_list, 'totalPage' => ceil($record_count / $size)]));
            } elseif ($type == 1) {
                $refound_list = return_order($order_id);  // 退货进度查询列表
                die(json_encode(['refound_list' => $refound_list, 'totalPage' => ceil($record_count / $size)]));
            }
        }
    ...
    public function actionSubmitReturn()
    {
        ...
        if ($query) {
            $ret_id = dao('order_return')->where(['return_sn' => $order_return['return_sn']])->getField('ret_id');
            ...
            show_message(L('Apply_Success_Prompt'), L('See_returnlist'), url('detail', ['ret_id' => $ret_id]), 'info', true, $order_return);
        } else {
            show_message(L('Apply_Abnormal'), '', '', 'info', true);
        }
    }
    ...
    public function actionImgReturn()
    {
        ...
        //上传图片并 获得路径
        $result = $this->upload('data/return_images', false, 2, [600, 600]);
    ...
    public function actionGetSpec()
    {
        if (IS_AJAX) {
            ...
            $rec_id = I('id', 0,['htmlspecialchars','trim']);
