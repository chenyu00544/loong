...
class AdminController extends BackendController
{
	...
	//////////////////////////////////////////////////////////
	/*发送模板消息  拼团成功模板消息*/
	public function actionSendTeamTemplateTest()
	{
		exit;
		
		//print_r($_REQUEST);
		//print_r($_SESSION);
		//https://cc.missmall.com/mobile/index.php?m=wechat&c=admin&a=send_team_template  
		//OK!!
		//echo "测试外部调用";exit;
		//oW-BJxCbiIK23TDFrHdWhkRoRE8g
		$openId = "oClKRv55RosqqnijqP0OrJkgWTeM";//"oW-BJxCbiIK23TDFrHdWhkRoRE8g";//"oClKRv2EcwQcSypiL7kYY8ugGtSg";//"oClKRv9xvUtHcsB7ayfAD4sHlOWw";
		$url = "https://www.missmall.com/mobile/index.php?m=team&c=free";
		 // 组合发送数据
		 $message_data = array();
        $message_data['touser'] = $openId;
        $message_data['template_id'] = "0HytLwhUzVh3L13Lx-YHQPAGvu0s7wHHc2X5z8yNHx8";//DMoIIWv93yQrVwcG_WquafDJUVnFtWcSxDIb6_fqWds  // 
		...
		*/
		...
		//（测试）
		$sendData['first'] = array(
									'value'=>"恭喜您获得一张拼团免单券",
									'color'=>"#FF0000"
									);
									
		$sendData['keyword1'] = array(
							'value'=>"拼团开团免单券",
							'color'=>"#FF0000"
							);	

		$sendData['keyword2'] = array(
							'value'=>$sendDate,
							'color'=>"#FF0000"
							);				

		$sendData['keyword3'] = array(
									'value'=>$sendDate,
									'color'=>"#FF0000"
									);

		$sendData['remark'] = array(
									'value'=>"免单券已成功领取，请在规定时间内使用",
									'color'=>"#FF0000"
									);									
		
        $message_data['data'] = $sendData;//unserialize();///这里可以序列化后存入数据库，这里暂时不存。
		
        $rs = $this->weObj->sendTemplateMessage($message_data);//urlencode($message_data)
		...
		else
		{
		  ...
		  //  dao('wechat_template_log')->data(array('msgid' => $rs['msgid']))->where(array('code' => $logs['code'], 'openid' => $logs['openid'], 'wechat_id' => $wechat_id))->save();
			...
			$serializeData = "拼团领券通知";//addslashes(serialize($message_data));//['data']urlencode
			
			$insertSql = 
			  "
				insert into {pre}wechat_template_log_my 
							(
								openid,
								data,
								url,
	...
	//send_team_template
	public function actionSendTeamTemplate()
	{
		...
		*/
		...
		// 优惠券权限
        $this->admin_priv('coupons_manage');
		//exit;
		 
		$randListStr = I('get.rand_list');
		$endTime = I('get.end_time');
		$isAll = I('get.is_all');
		
		//类型 1表示两个都发  2表示就公众号  3表示就小程序
		
		
		//$cou_type = isset($_POST['cou_type']) ? intval($_POST['cou_type']) : 0;
		$sendType = I('get.st');//isset(I('get.st'))? intval(I('get.st')) : 0;
		...
		$getSiteUrlSql = 
			"
				select value from {pre}shop_config  where code = 'site_domain' limit 0,1 
			";
		
		$urlT =  $this->model->query($getSiteUrlSql);
		
		$siteUrl = $urlT[0]['value'];
		
		$url = $siteUrl."mobile/index.php?m=team&c=free";
		...
		if($sendWx)
		{
			if(!empty($isAll))
			{
				...
				$allUserSql = 
				"
					select openid,nickname,ect_uid from {pre}wechat_user 
				";
			}
			else
			{
				/////系统的其实二次获取分数区间的 不是直接user_rank字段 
				
				$getAllRankSql = 
				"
					select  min_points, max_points, special_rank from {pre}user_rank where rank_id in ($randListStr)
				";
				...
				//select a.openid,a.nickname from {pre}wechat_user as a left join {pre}users as b on a.ect_uid = b.user_id  where b.user_rank in ($randList)
				
				$allUserSql = 
				"
					select a.openid,a.nickname,a.ect_uid from {pre}wechat_user as a left join {pre}users as b on a.ect_uid = b.user_id where 1 $whereRankSql
				";
			}
			...
			$getTempIdSql = 
				"
					select template_id from {pre}wechat_template_my  where is_team_buy = '1' limit 0,1 
				";
			
			$wx_template_idT =  $this->model->query($getTempIdSql);
			
			$wx_template_id = $wx_template_idT[0]['template_id'];
			...
			for($i=0;$i<count($userList);$i++)
			{
					...
					$message_data = array();
					$message_data['touser'] = $openId;
					$message_data['template_id'] = trim($wx_template_id);//"Bk7dwZbWLraMFL2E-bnkvUHCHJ38ne_DEjVdNkbw0lo";//DMoIIWv93yQrVwcG_WquafDJUVnFtWcSxDIb6_fqWds  // 
					$message_data['url'] = $url;
					$message_data['topcolor'] = '#FF0000';
		...
		if($sendWxApp)
		{
			...
			for($i=0;$i<count($userList);$i++)
			{
					...
					$message_data = array();
					$message_data['touser'] = $openId;
					$message_data['template_id'] = "qiFZr46-pBJNd26Xul2MLz333qaoKZc0ETIKXIi4l18";//$templateWxId;
					...
					$sendData['keyword4'] = array(
												'value'=>$endTime.'前',
												'color'=>"#000000"
												);

					$sendData['keyword5'] = array(
												'value'=>"免单券已成功领取，请在规定时间内使用",
												'color'=>"#000000"
												);		
					...
					if (!empty($result)) 
					{
						...
						$this->model->query($okSql);
						 ...
						 $msgid = '';//$result['msgid'];
							...
							$serializeData = "拼团免单券";//$templateName;//addslashes(serialize($message_data));//['data']urlencode
		...
		//https://cc.missmall.com/miss/coupons.php?act=add//去掉最后add 三个字母，加上list 。
		//print_r($_SERVER) ;
		//'coupons.php?act=list';
		//redirect($_SERVER['HTTP_REFERER']);
		
		//echo substr($_SERVER['HTTP_REFERER'],0,-3).'list';
		
		redirect($siteUrl.'miss/coupons.php?act=list');
		/**/
		//exit;
	 ...
	 public function actionSendOneMessage()
	 {
		...
		$scene= $_GET['scene'];//'120'; //因为是从其他页面传过来的所以用GET的方法接受，当然你也可以麻烦点自己写成POST方法比较安全
		...
		'url'=>"http://cc.missmall.com", //自己网站链接url 
		'scene'=>"$scene",
		'title'=>"订阅消息测试", //标题
		'data'=>array
			(
		...
		$rs = $this->weObj->sendOneMessage($data);
	...
	/////////////////////////////////////////////
	//小程序模板
	 public function actionTemplateWxappMy()
    {
        ...
        $list = $this->model->table('wechat_wxapp_template_my')->where($condition)->order(' status desc , id asc')->select();
	...
	public function actionTemplateLogMy()
    {
		...
		$sql = 
		"
			select a.data,a.send_time,b.user_id,b.nick_name from {pre}wechat_template_log_my as a left join {pre}users as b on a.user_id = b.user_id order by a.send_time desc limit 0,20 
		"; 
    ...
    public function actionTemplateMy()
    {
        ...
        $list = $this->model->table('wechat_template_my')->where($condition)->order(' status desc , id asc')->select();
	 ...
	 //////get_temp_vars//ajax解析变量
	  public function actionGetTempVars()
	  {
		   $this->admin_priv('template');
		   
		   if (IS_AJAX) 
		   {
            $data = I('post.template_content', '', 'trim');
			
            if ($data) 
			{
                $result = array();
				preg_match_all("/{{(.*?).DATA}}/i",$data, $result); //重要，加了一个问号
	
				$str = implode(',',$result[1]); 
			   
                exit(json_encode(array('status' => 1,'vars'=>$str)));
            }
			else 
			{
                exit(json_encode(array('status' => 0,'msg'=>'请输入内容')));
            }
          }
	  }
	
	
	//////////含：添加  更新   （init 、handler）
	 public function actionEditTemplateMy()
	 {
		//  $this->admin_priv('template');
		//  echo "actionAddTemplateMy";
		// $this->display();
        ...
        if (IS_POST)// || IS_AJAX 
		{
           ...
           // $data = I('post.data');
            ...
            $data['title'] = I('post.template_title');
			$data['template'] = I('post.template_content');
			$data['demo_content'] = I('post.demo_content');
			$data['status'] = I('post.template_status');
			$data['template_id'] = I('post.template_id');
			$data['sort'] = I('post.template_sort');
			$data['wechat_id'] = '1';
			
			 $result = array();
			 
			 preg_match_all("/{{(.*?).DATA}}/i",$data['template'], $result); //重要，加了一个问号
            ...
            if (!empty($id)) 
			{
                $this->model->table('wechat_template_my')->data($data)->where(array('id' => $id))->save();
            }
			else 
			{
                // 添加
                $this->model->table('wechat_template_my')->data($data)->add();
            }
		...
		if(!empty($id))
		{
			 $sql = 'select * from {pre}wechat_template_my  
	...
	*/
     ...
     * 开关按钮
     */
    public function actionSwitchMy()
    {
        // 模板消息权限
        $this->admin_priv('template');
        ...
        if ($status == 1) {
            // 模板ID为空
            $template = $this->model->table('wechat_template_my')->field('template_id, code')->where($condition)->find();
            if (empty($template['template_id'])) {
                $template_id = $this->weObj->addTemplateMessage($template['code']);
                // 已经存在模板ID
                if ($template_id) {
                    $data['template_id'] = $template_id;
                    $this->model->table('wechat_template_my')->data($data)->where($condition)->save();
                } else {
                    $this->message($this->weObj->errMsg, null, 2);
                }
            }
	...
	public function actionSendTemplateMy()
	{
		...
		if(IS_POST)
		{
			...
			$templateWxId = trim($list[0]['template_id']);
			
			$templateName = $list[0]['title'];
			...
			else
			{
				...
				//select  openid,nickname,ect_uid from {pre}wechat_user where 1 $whereRankSql
				$allUserSql = 
				"
					select a.openid,a.nickname,a.ect_uid from {pre}wechat_user as a left join {pre}users as b on a.ect_uid = b.user_id  where 1 $whereRankSql
				";
				...
				$allUserSql = 
				"
					select a.openid,a.nickname,a.ect_uid from {pre}wechat_user as a left join {pre}users as b on a.ect_uid = b.user_id  where b.user_rank = '$send_group' 
				";
				*/
			}
			...
			for($i=0;$i<count($userList);$i++)
			{
					...
					$message_data = array();
					$message_data['touser'] = $openId;
					$message_data['template_id'] = $templateWxId;//DMoIIWv93yQrVwcG_WquafDJUVnFtWcSxDIb6_fqWds  // 
					$message_data['url'] = $url;
					$message_data['topcolor'] = '#FF0000';
			...
			//redirect(substr($_SERVER['HTTP_REFERER'],0,-3).'list');
