...
class IndexController extends FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (!empty($scene_id)) {
            ...
            $sceneArr = explode('#', $scene_id);
            if ($sceneArr[0] == 'bargain') {
                $re_bargain = dao('bargain_goods')->where(['id' => $sceneArr[1]])->find();
                $re_goods = dao('goods')->where(['goods_id' => $re_bargain['goods_id']])->find();
                $bargainInfo = [
                    '0' => [
                        'Title' => $re_goods['goods_name'],
                        'Description' => $re_bargain['bargain_desc'] . '',
                        'PicUrl' => get_image_path($re_goods['goods_img']),
                        'Url' => 'https://www.missmall.com/mobile/bargain/goods/index/id/' . $sceneArr[1] . '.html'
                    ],
                ];

                $this->weObj->news($bargainInfo)->reply();
//                dao('zz_wx_test_log')->data(['content' => json_encode($bargainInfo)])->add();
            }
        }
    ...
    private function subscribe($openid = '', $scene_id = '')
    {
        if (!empty($openid)) {
            ...
            // 未关注
            if (empty($result)) {
                if ($this->ru_id == 0 || $this->ru_id == 6273) {
                    // 查询推荐人ID
    ...
    public static function snsapi_base($ru_id = 0)
    {
        ...
        if (!empty($wxinfo) && !empty($wxinfo['appid']) && is_wechat_browser() && isset($_COOKIE['ectouch_ru_id']) && !empty($_COOKIE['ectouch_ru_id']) && empty($_SESSION['seller_openid'])) {
            $config = [
                'appid' => $wxinfo['appid'],
