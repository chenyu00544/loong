...
class Admin extends PluginController
{
    ...
    public function marketUsers()
    {
        ...
        if (empty($user_id)) {
            ...
            $sql = "SELECT id, nickname, sex, headimg, status, addtime, checktime, wechatname FROM {pre}wechat_wall_user WHERE wall_id = '" . $market_id . "' AND wechat_id = '" .$this->wechat_id. "'  ORDER BY addtime DESC limit $offset";
            $list = $this->model->query($sql);
            if ($list[0]['id']) {
                foreach ($list as $k => $v) {
                    if ($v['sex'] == 1) {
    ...
    public function marketQrcode()
    {
        ...
        if (!empty($market_id)) {
            $url = __STATIC__ . url('wechat/index/market_show', ['type' => 'wall', 'function' => 'wall_user_wechat', 'wall_id' => $market_id, 'ru_id' => $this->ru_id]);
        ...
        if (IS_AJAX || IS_POST) {
            ...
            if (IS_POST && $handler && $handler == 'batch_checking') {
                ...
                if ($function == 'messages' && !empty($messagelist) && is_array($messagelist)) {
                    ...
                    if ($num > 50) {
                        $this->message('批量处理数量一次不能超过50', null, 2, $this->ru_id);
                    }
