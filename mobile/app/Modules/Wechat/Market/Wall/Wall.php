...
class Wall extends PluginController
{
    ...
    public function actionWallUser()
    {
        if (IS_AJAX) {
            ...
            $qrcode = dirname(ROOT_PATH) .'/data/attached/wall/wall_qrcode_' . $wall_id. '.png';
            $water_logo = ROOT_PATH . 'public/img/shop_app_icon.png';

            $url = __STATIC__ . url('wechat/index/market_show', ['type' => 'wall', 'function' => 'wall_user_wechat', 'wall_id' => $wall_id, 'ru_id' => $this->ru_id]);
     ...
     * 获取上墙用户（审核通过用户）
     */
    public function actionGetWallUsers()
    {
        if (IS_AJAX) {
            $result['errCode'] = 0;
            $result['errMsg'] = '';
            ...
            // 用户
            $sql = "SELECT nickname, headimg, id, wechatname, headimgurl FROM {pre}wechat_wall_user WHERE wall_id = " . $wall_id . " AND status = 1 GROUP by openid ORDER BY addtime DESC";
            $users = $this->model->query($sql);
            if (empty($users)) {
                $result['errCode'] = 2;
                $result['errMsg'] = '暂无参与用户';
                exit(json_encode($result));
            }
     ...
     * 抽取的动作
     */
    public function actionStartSelect()
    {
        if (IS_AJAX) {
            $result['errCode'] = 0;
            $result['errMsg'] = '';
            ...
            $sql = "SELECT nickname, headimg, openid, id, wechatname, headimgurl FROM {pre}wechat_wall_user WHERE wall_id = " . $wall_id . " AND status = 1 GROUP by openid ORDER BY addtime DESC";
            $list = $this->model->query($sql);
            if ($list) {
                // 随机
                $key = mt_rand(0, count($list) - 1);
