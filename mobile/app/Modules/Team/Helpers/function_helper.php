...
function team_goods_log2($goods_id = 0)
{
    $sql = "select tl.team_id, tl.start_time,o.team_parent_id,g.goods_id,tg.validity_time ,tg.team_num from 
	" . $GLOBALS['ecs']->table('team_log') . " as tl LEFT JOIN " . $GLOBALS['ecs']->table('order_info') . " 
	as o ON tl.team_id = o.team_id LEFT JOIN  " . $GLOBALS['ecs']->table('goods') . " as g ON tl.goods_id = g.goods_id
	LEFT JOIN " . $GLOBALS['ecs']->table('team_goods') . " AS tg ON tl.goods_id = tg.goods_id  " . "
	LEFT JOIN " . $GLOBALS['ecs']->table('coupons_user') . " AS cou  ON o.order_id = cou.order_id   " . "
	where tl.goods_id = $goods_id and tl.status <1 and tl.is_show = 1 and o.extension_code ='team_buy' and o.team_parent_id > 0 and pay_status = 2 and cou.is_use = 0";
 ...
 */
 ...
 function team_goods_log2($goods_id = 0)
{
	 $sql = "select cou.is_use,tl.team_id, tl.start_time,o.team_parent_id,g.goods_id,tg.validity_time ,tg.team_num from " . 
	$GLOBALS['ecs']->table('team_log') . " as tl LEFT JOIN " . $GLOBALS['ecs']->table('order_info') .
	" as o ON tl.team_id = o.team_id LEFT JOIN  " . $GLOBALS['ecs']->table('goods') . " as g ON tl.goods_id = g.goods_id LEFT JOIN " 
	. $GLOBALS['ecs']->table('team_goods') . " AS tg ON tl.t_id = tg.id  " . 
	
	" LEFT JOIN " . $GLOBALS['ecs']->table('coupons_user') . " AS cou  ON o.order_id = cou.order_id   ".
	
	" where tl.goods_id = $goods_id and
	tl.status <1 and tl.is_show = 1 and o.extension_code ='team_buy' and o.team_parent_id > 0 and pay_status = 2 and tg.is_team = 1 ";
	
	// and cou.is_use = 0   改成了and cou.is_use != 1  默认是空的    
	//这里先获取。然后过滤  不加条件
	//echo  $sql;
...
function my_team_goods($user_id, $type = 1, $page = 1, $size = 10)
{
    ...
    foreach ($res as $key => $vo) {
        ...
        $goods[$key]['team_url'] = url('team/goods/teamwait', ['team_id' => $vo['team_id'], 'user_id' => $_SESSION['user_id']]);//查看团
        if ($vo['status'] == 1) {
...
////////////////////////////////评团可用优惠券
function get_user_pintuan_coupons_list_goods($user_id = '', $goodsId)
{
    $time = gmtime();
	...
	//这里最后去掉了 来源  是商家还是平台。??     " . $n . " >= c.cou_man AND
                $where = " WHERE cu.is_use=0 AND (c.cou_type = 95 || cou_type = 96) AND c.cou_end_time > $time AND $time>c.cou_start_time AND cu.user_id ='$user_id'
                        AND (c.cou_goods =0 OR FIND_IN_SET('" . $goodsId . "',c.cou_goods)) ";/////AND c.ru_id='" . $v['ru_id'] . "'
						
                $sql = " SELECT c.*,cu.*,o.order_sn,o.add_time FROM " . $GLOBALS['ecs']->table('coupons_user') . " cu LEFT JOIN " 
				. $GLOBALS['ecs']->table('coupons') . " c ON c.cou_id=cu.cou_id LEFT JOIN " 
				. $GLOBALS['ecs']->table('order_info') . " o ON cu.order_id=o.order_id $where ";
...
////////////////////////////////评团可用优惠券
function get_user_pintuan_coupons_list($user_id = '', $is_use = false, $total = false, $cart_goods = false, $user = true, $num = 10)
{
    $time = gmtime();

    //可使用的(平台用平台发的,商家用商家发的,当订单中混合了平台与商家的商品时,各自计算各自的商品总价是否达到各自发放的优惠券门槛,达到的话当前整个订单即可使用该优惠券)
    if ($is_use && $total && $cart_goods)//$is_use && $total && $cart_goods 
	{
		//echo "1111111111<br/>";
        ...
        foreach ($cart_goods as $k => $v) 
		{
			...
			{
				//这里最后去掉了 来源  是商家还是平台。??   AND " . $n . " >= c.cou_man
                $where = " WHERE cu.is_use=0 AND (c.cou_type = 95 || cou_type = 96) AND c.cou_end_time > $time AND $time>c.cou_start_time  AND cu.user_id ='$user_id'
	...
	else 
	{
		...
		{
            $where = " WHERE cu.user_id IN(" . $user_id . ") AND cu.is_use=0 AND (c.cou_type = 95 || c.cou_type = 96) ";
        } elseif (!empty($user_id)) {
            $where = " WHERE cu.user_id IN(" . $user_id . ") AND cu.is_use=0 AND (c.cou_type = 95 || c.cou_type = 96) GROUP BY c.cou_id";
        }
