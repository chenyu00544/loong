...
class FlowController extends FrontendController
{
    ...
    public function actionIndex()
    {
		...
		if (empty($_SESSION['team_id'])) 
		{
			...
			{
				//free_team_count
				$sql = "SELECT free_team_count FROM " . $this->ecs->table('users') . " WHERE user_id = " . $_SESSION ['user_id'];
				$free_team_count = $this->db->getOne($sql);
				$free_team_count = $this->getNum($free_team_count);
				
				if($free_team_count > 0)
				{
					$has_free_count = true;
				}
			}
		...
		*/
		...
		if (empty($_SESSION['team_id'])) 
		{
			if ($_SESSION ['user_id']) 
			{
				 $user_coupons = get_user_pintuan_coupons_list($_SESSION['user_id'], true, $total_goods_price, $cart_goods, true);
				
				if (!empty($user_coupons)) 
				{
					//$order['cou_id'] = $user_coupons[0]['uc_id'];//$user_coupons[0]['cou_id'];//不是cou。
					$has_free_count = true;
				}
			}
		}
		...
		$this->assign('has_free_count', $has_free_count);
		//////////////////////////////////////////////////////////////////	
    ...
    public function actionDone()
    {
		...
		if (empty($_SESSION['team_id']))
		{
			if ($_SESSION ['user_id']) 
			{
				...
				if($free_team_count > 0) 
				{
					...
					//////并且数量减去1  
					$sql = "update ". $this->ecs->table('users') ." set free_team_count = free_team_count - 1 WHERE user_id =". $_SESSION ['user_id'];
					$this->db->query($sql);
		...
		if (empty($_SESSION['team_id']))
		{
			if ($_SESSION ['user_id']) 
			{
				...
				if (!empty($user_coupons)) 
				{
					...
					//$sql = "update ". $this->ecs->table('coupons_user') ." set is_use = 1,is_use_time = $useTime  WHERE uc_id =".$uc_id;
		...
		if($hasPtFree)
		{
			...
			$sql = "update ". $this->ecs->table('coupons_user') ." set is_use = 1,is_use_time = $useTime,order_id='$ptOrderId'  WHERE uc_id =".$ptUcId;
        ...
        if (is_dir(APP_WECHAT_PATH)) {
            $pushData = [
                'orderID' => ['value' => $order['order_sn']], //订单号
                'orderMoneySum' => ['value' => $order['order_amount']], //订单金额
