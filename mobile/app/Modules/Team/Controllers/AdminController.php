...
class AdminController extends BackendController
{
    ...
    public function actionIndex()
    {
        ...
        $sql = "SELECT tg.*,g.user_id,g.goods_sn,g.goods_name,g.shop_price,g.market_price,g.goods_number,g.goods_img,g.goods_thumb,g.is_best,g.is_new,g.is_hot,tg.sort_order FROM {pre}team_goods as tg LEFT JOIN {pre}goods as g ON tg.goods_id = g.goods_id WHERE " . $where . " ORDER BY tg.id DESC LIMIT " . $offset;
        ...
        foreach ($list as $key => $val) {
            ...
            $list[$key]['sort_order'] = $val['sort_order'];
            if ($val['is_audit'] == 1) {
                $is_audit = '审核未通过';
    ...
    public function actionAddgoods()
    {
        ...
        if (I('id')) {
            $id = I('id', '', 'intval');
            $info = $this->model->table('team_goods')->where(['id' => $id])->find();//拼团商品信息
    ...
    public function actionEditgoods()
    {
        ...
        if($type == 'sort_order'){
            $sql = "UPDATE {pre}team_goods SET sort_order = $sortorder WHERE goods_id = $goods_id ";
        }
