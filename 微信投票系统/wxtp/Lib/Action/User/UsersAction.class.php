...

{
    ...
    public function index()
    {
        ...
        $user_group = M('User_group')->field('id,name')->order('id desc')->select();

        $agentid = 'agentid = 0';

        if (isset($_GET['agentid'])) {

            $agentid = array('agentid' => intval($_GET['agentid']));

        }

        if (isset($_GET['inviter'])) {

            $agentid = array('inviter' => intval($_GET['inviter']));

        }

        $count = $users->where($agentid)->count();

        $pageM = new Page($count, 25);

        $pageShow = $pageM->show();

        $IIIIIIIIlIII = $users->where($agentid)->order('id desc')->limit($pageM->firstRow . ',' . $pageM->listRows)->select();
        foreach ($user_group as $IIIIIIIlI11I => $IIIIIIl1llIl) {
...

    {
        ...
        $agentid['uid'] = $IIIIIIIII1I1;

        $IIIIIllII1lI = M('wxuser')->where($agentid)->count();
