    ...
    <div class="warpper clearfix">
        ...
        <div class="content">
            ...
            <div class="fromlist clearfix">
                <div class="main-info">
                    <form name="alidayu" action="{{url('admin/alisms/'.$alisms->id)}}" method="post"
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" name="set_sign" class="form-control" value="{{$alisms->set_sign}}"
                                       placeholder="与阿里短信内短信签名保持一致">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-4 control-label"><b>*</b>短信模板：</label>
                            <div class="col-sm-4">
                                <input type="text" name="temp_id" class="form-control" value="{{$alisms->temp_id}}"
                                       placeholder="与阿里短信内短信模板code保持一致">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-4 n-wd400">
                                <textarea name="temp_content" class="form-control ww" row="5"
                                          placeholder="与阿里短信内短信签名保持一致"
                                          style="min-height:100px;">{{$alisms->temp_content}}</textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <select name="send_time" class="form-control select">
                                    ...
                                    @foreach($sendTime as $key => $val)
                                        <option value="{{$key}}"
                                                @if($key == $alisms->send_time) selected @endif>{{$val}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
