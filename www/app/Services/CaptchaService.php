...
{
    protected static $seKey = 'vcvbuy.com';// 验证码加密密钥
    protected static $codeSet = '2345678abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY';// 验证码字符集合
    protected static $expire = 1800;// 验证码过期时间（s）
    protected static $useZh = false;// 使用中文验证码
    ...
    protected static $useImgBg = false;// 使用背景图片
    protected static $fontSize = 16;// 验证码字体大小(px)
    protected static $useCurve = true;// 是否画混淆曲线
    protected static $useNoise = true;// 是否添加杂点
    protected static $imageH = 37;// 验证码图片高度
    protected static $imageW = 112;// 验证码图片宽度
    protected static $length = 4;// 验证码位数
    protected static $fontttf = '';// 验证码字体，不设置随机获取
    protected static $bg = array(243, 251, 254);// 背景颜色
    protected static $reset = true;// 验证成功后是否重置
    ...
    public static function entry()
    {
        ...
        $ttfPath = base_path() . '/public/styles/verify/' . (self::$useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        self::$fontttf = $ttfPath . self::$fontttf;
        ...
        if (self::$useZh) {
            ...
            for ($i = 0; $i < self::$length; $i++) {
                $code[$i] = iconv_substr(self::$zhSet, floor(mt_rand(0, mb_strlen(self::$zhSet, 'utf-8') - 1)), 1, 'utf-8');
                imagettftext(self::$_image, self::$fontSize, mt_rand(-40, 40), self::$fontSize * ($i + 1) * 1.5, self::$fontSize + mt_rand(10, 20), self::$_color, self::$fontttf, $code[$i]);
            }
        } else {
            for ($i = 0; $i < self::$length; $i++) {
                $code[$i] = self::$codeSet[mt_rand(0, strlen(self::$codeSet) - 1)];
                $codeNX += mt_rand(self::$fontSize * 1.2, self::$fontSize * 1.6);
                imagettftext(self::$_image, self::$fontSize, mt_rand(-40, 40), $codeNX, self::$fontSize * 1.6, self::$_color, self::$fontttf, $code[$i]);
            }
        }

        // 保存验证码
        $key = self::authcode(self::$seKey);
        $code = self::authcode(strtoupper(implode('', $code)));
        $secode = array();
        $secode['verify_code'] = $code; // 把校验码保存到session
        $secode['verify_time'] = time(); // 验证码创建时间
        Cache::put($key, $secode, self::$expire / 60);
    ...
    private static function _writeNoise()
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            for ($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring(self::$_image, 5, mt_rand(-10, self::$imageW), mt_rand(-10, self::$imageH), $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
    ...
    private static function _background()
    {
        ...
        @imagecopyresampled(self::$_image, $bgImage, 0, 0, 0, 0, self::$imageW, self::$imageH, $width, $height);
     ...
     */
    public static function check($code = '', $seKey = '', $type = '')
    {
        $now_time = time();
        if ($seKey != '') {
            $key = self::authcode($seKey);
        } else {
            $key = self::authcode(self::$seKey);
        }
        // 验证码不能为空
        ...
        // session 过期
        if ($now_time - $secode['verify_time'] > self::$expire) {
