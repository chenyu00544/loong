...
class CaptchaController extends CommonController
{
    ...
    protected $fontSize = 16;// 验证码字体大小(px)
    protected $useCurve = true;// 是否画混淆曲线
    protected $useNoise = true;// 是否添加杂点
    protected $imageH = 37;// 验证码图片高度
    protected $imageW = 112;// 验证码图片宽度
    protected $length = 4;// 验证码位数
    protected $fontttf = '';// 验证码字体，不设置随机获取
    protected $bg = array(243, 251, 254);// 背景颜色
    protected $reset = true;// 验证成功后是否重置
    ...
    public function entry()
    {
        ...
        $ttfPath = base_path() . '/public/styles/verify/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        Cache::put($key, $secode, $this->expire / 60);
     ...
     */
    public function check($code = '', $seKey = '', $type = '')
    {
        $now_time = time();
        if ($seKey != '') {
