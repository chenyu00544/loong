...
class AlidayuRepository implements AlidayuRepositoryInterface
{
    ...
    public function getAlidayu($id)
    {
        $where['id'] = $id;
        return $this->alidayuConfigureModel->getAlidayu($where);
    }

    public function setAlidayu($data, $id)
    {
        $req = ['code' => 5, 'msg' => '操作失败'];
        $where['id'] = $id;
        $updata = $data;
        $re = $this->alidayuConfigureModel->setAlidayu($where, $updata);
        if ($re) {
            $req['code'] = 1;
            $req['msg'] = '操作成功';
        }
    ...
    {
        $data['add_time'] = time();
        return $this->alidayuConfigureModel->addAlidayu($data);
    }

    public function delAlidayu($id)
    {
        $req = ['code' => 5, 'msg' => '操作失败'];
        $where['id'] = $id;
        $re = $this->alidayuConfigureModel->delAlidayu($where);
        if ($re) {
            $req['code'] = 1;
            $req['msg'] = '操作成功';
        }
