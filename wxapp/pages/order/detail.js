...
Page({
  ...
  pay_order: function (e) {
    ...
    var order_id = this.data.orders.order_id
  ...
  getTransport:function(){
    var that = this;
    if (!that.data.orders.invoice_no){
      return;
    }
    wx.request({
      url: app.apiUrl('user/order/transport'),
      data: {
        relname: that.data.orders.shipping_name,
        order_sn: that.data.orders.invoice_no
      },
      header: {
        'Content-Type': 'application/json',
        'X-ECTouch-Authorization': token
