  ...
  wx.request({
    ...
    success: function (res) 
    {
      ...
      that.setData({
        ...
        wxapp_ad1: res.data.ad1,
        wxapp_ad2: res.data.ad2,
        wxapp_ad3: res.data.ad3,
        wxapp_ad4: res.data.ad4,
        wxapp_ad5: res.data.ad5,
        wxapp_ad6: res.data.ad6,

        store_wxapp_ads: res.data.ad_list
      });
      // page++;
      that.setData({
        hidden: true
...
Page({
  ...
  siteDetail: function (e) {
    ...
    if (cash != ""){
      wx.navigateTo({
        url: "../cashgoods/index?objectId=" + goodsId
      });
    }else{
      wx.navigateTo({
        url: "../goods/index?objectId=" + goodsId + "&cash=" + cash
      });
    }
  },
  //分类弹框
  ...
  onShareAppMessage: function () {
    var title = "店铺"
    if (this.store_data.detail.rz_shopName){
      title = this.store_data.detail.rz_shopName;
    }
    return {
      title: title,
      desc: '小程序本身无需下载，无需注册，不占用手机内存，可以跨平台使用，响应迅速，体验接近原生App',
      path: '/pages/store/index?objectId=' + wx.getStorageSync('user_id')
    }
  },
