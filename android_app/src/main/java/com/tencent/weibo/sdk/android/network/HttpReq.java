...
public abstract class HttpReq extends AsyncTask<Void, Integer, Object> {
    ...
    public Object runReq() throws Exception {
        ...
        } else if (this.mParam.getmParams().get(ShareActivity.KEY_PIC) != null) {
            return processResponse(picMethod());
        } else {
            getMethod = new UTF8PostMethod(this.mUrl);
        }
        httpClient.getHostConfiguration().setHost(this.mHost, this.mPort, HttpConstant.HTTPS);
        getMethod.setRequestHeader(HttpRequest.HEADER_CONTENT_TYPE, HttpRequest.CONTENT_TYPE_FORM);
        setReq(getMethod);
    ...
    private InputStream picMethod() {
        ...
        httpPost.setHeader(HttpRequest.HEADER_CONTENT_TYPE, "multipart/form-data" + "; boundary=" + "---------------------------7da2137580612");
        if (reqParam != null) {
            try {
                if (!reqParam.equals("")) {
                    for (String str2 : reqParam.split("&")) {
                        if (!(str2 == null || str2.equals("") || str2.indexOf("=") <= -1)) {
                            ...
                            stringBuilder.append("Content-Disposition:form-data; name=\"" + split[0] + "\"" + "\r\n");
                    ...
                    char[] toCharArray = ((String) this.mParam.getmParams().get(ShareActivity.KEY_PIC)).toCharArray();
