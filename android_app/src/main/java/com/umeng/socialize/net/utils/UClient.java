...
public class UClient {
    ...
    private JSONObject httpGetRequest(URequest uRequest) {
        ...
        try {
            ...
            try {
                ...
                try {
                    Log.e("UMhttprequest", "error:" + exception.getMessage());
                    if (httpURLConnection != null) {
                        httpURLConnection.disconnect();
                    }
                    return httpURLConnection2;
    ...
    private void addFilePart(StringBuilder stringBuilder, String str, byte[] bArr, String str2, OutputStream outputStream) throws IOException {
        stringBuilder.append("--").append(str2).append(END).append("Content-Disposition: form-data; name=\"").append(str).append("\"; filename=\"").append(str).append("\"").append(END).append("Content-Type: ").append("application/octet-stream").append(END).append("Content-Transfer-Encoding: binary").append(END).append(END);
