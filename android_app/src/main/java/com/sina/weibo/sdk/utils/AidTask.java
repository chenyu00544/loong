...
public class AidTask {
    ...
    class AidTask_1 implements Runnable {
        AidTask_1() {
        }
    ...
    class AidTask_2 implements Runnable {
        AidTask_2() {
        }
    ...
    private AidTask(Context context) {
        ...
        new Thread(new AidTask_1()).start();
    }
    ...
    private void initAidInfo(String str) {
        if (!TextUtils.isEmpty(str)) {
            this.mAppKey = str;
            new Thread(new AidTask_2()).start();
        }
    }
    ...
    private String loadAidFromNet() throws WeiboException {
        ...
        try {
            str = NetUtils.internalHttpRequest(this.mContext, "https://api.weibo.com/oauth2/getaid.json", "GET", weiboParameters);
