...
public class SsoHandler {
    ...
    private ServiceConnection mConnection = new SsoHandler_1();
    private int mSSOAuthRequestCode;
    private WebAuthHandler mWebAuthHandler;
    private WeiboInfo mWeiboInfo;

    class SsoHandler_1 implements ServiceConnection {
        SsoHandler_1() {
        }
        ...
        if (i == this.mSSOAuthRequestCode) {
            if (i2 == -1) {
                ...
                if (intent != null) {
                    ...
                    this.mAuthListener.onWeiboException(new WeiboDialogException(intent.getStringExtra("error"), intent.getIntExtra(SDKInitializer.SDK_BROADTCAST_INTENT_EXTRA_INFO_KEY_ERROR_CODE, -1), intent.getStringExtra("failing_url")));
                    return;
                }
                LogUtil.d(TAG, "Login canceled by user.");
                this.mAuthListener.onCancel();
            ...
            if (i2 == -1) {
                ...
                if (intent != null) {
                    ...
                    if (stringExtra != null) {
                        this.mAuthListener.onWeiboException(new WeiboDialogException(intent.getStringExtra("error"), intent.getIntExtra(SDKInitializer.SDK_BROADTCAST_INTENT_EXTRA_INFO_KEY_ERROR_CODE, -1), intent.getStringExtra("error_description")));
                        return;
                    }
                    return;
                }
