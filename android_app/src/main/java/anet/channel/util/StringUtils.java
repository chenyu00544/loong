...
public class StringUtils {
        ...
        try {
            for (Entry entry : map.entrySet()) {
                if (entry.getKey() != null) {
                    stringBuilder.append(URLEncoder.encode((String) entry.getKey(), str)).append("=").append(URLEncoder.encode(stringNull2Empty((String) entry.getValue()), str).replace(SocializeConstants.OP_DIVIDER_PLUS, "%20")).append("&");
                }
            }
    ...
    private static String bytesToHexString(byte[] bArr, char[] cArr) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            int i3 = i + 1;
            cArr2[i] = cArr[(bArr[i2] & SocializeConstants.MASK_USER_CENTER_HIDE_AREA) >>> 4];
            i = i3 + 1;
