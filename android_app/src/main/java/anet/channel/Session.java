...
public abstract class Session implements Comparable<Session> {
    ...
    public boolean sameSession(Session session) {
        if (session != null && this.mIp != null && this.mPort == session.mPort && this.mConnType == session.mConnType && this.mIp.equals(session.mIp)) {
            return true;
        }
        return false;
    }
    ...
    protected void setPingTimeout() {
        ...
        cancelTimeout();
        if (this.mRecvTimeOutRunnable != null) {
            this.timeoutTaskFuture = c.a(this.mRecvTimeOutRunnable, 40000, TimeUnit.MILLISECONDS);
        }
    }
