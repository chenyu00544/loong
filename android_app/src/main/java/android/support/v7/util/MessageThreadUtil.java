...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new MessageThreadUtil_1_1();
            private final MessageQueue mQueue = new MessageQueue();

            class MessageThreadUtil_1_1 implements Runnable {
                MessageThreadUtil_1_1() {
                }

                public void run() {
                    SyncQueueItem next = MessageThreadUtil_1.this.mQueue.next();
                    while (next != null) {
                        switch (next.what) {
                            case 1:
                            ...
                            case 3:
                                ...
                                break;
                            default:
                                Log.e(e.TAG, "Unsupported message, what=" + next.what);
                                break;
                        }
                        next = MessageThreadUtil_1.this.mQueue.next();
                    }
                }
            }
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new MessageThreadUtil_2_1();
            AtomicBoolean mBackgroundRunning = new AtomicBoolean(false);
            private final Executor mExecutor = ParallelExecutorCompat.getParallelExecutor();
            private final MessageQueue mQueue = new MessageQueue();

            class MessageThreadUtil_2_1 implements Runnable {
                MessageThreadUtil_2_1() {
                }

                public void run() {
                    while (true) {
                        SyncQueueItem next = MessageThreadUtil_2.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    MessageThreadUtil_2.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    MessageThreadUtil_2.this.mQueue.removeMessages(2);
                                    MessageThreadUtil_2.this.mQueue.removeMessages(3);
                        ...
                        MessageThreadUtil_2.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
