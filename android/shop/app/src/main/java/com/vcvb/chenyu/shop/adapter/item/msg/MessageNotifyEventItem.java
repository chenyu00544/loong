...
public class MessageNotifyEventItem extends BaseItem<List<NotifyMsgEvent>> {
    public static final int TYPE = 3;

    public MessageNotifyEventItem(List<NotifyMsgEvent> bean, Context c) {
        super(bean, c);
    }
    ...
    public void onBindViewHolder(CYCBaseViewHolder holder, int position) {
        ...
        Glide.with(context).load(R.drawable.icon_notify).into(iv);
        int k = 0;
        NotifyMsgEvent notifyMsgEvent = new NotifyMsgEvent();
        for (int i = 0; i < mData.size(); i++) {
            if (!mData.get(i).isIs_look()) {
                k++;
            }
            if (i == mData.size() - 1) {
                notifyMsgEvent = mData.get(i);
            }
        }
        if (k > 0) {
            String str = "%d";
        ...
        if(notifyMsgEvent.getDescribe() != null){
            TextView title = holder.get(R.id.textView218);
            title.setText(notifyMsgEvent.getDescribe());
        }
        if(notifyMsgEvent.getContent() != null){
