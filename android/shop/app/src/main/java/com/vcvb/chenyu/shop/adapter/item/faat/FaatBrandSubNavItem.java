...
public class FaatBrandSubNavItem extends BaseItem<BrandNav> {
    public static final int TYPE = 1;
    public int num = 0;

    public FaatBrandSubNavItem(BrandNav bean, Context c, int num) {
        super(bean, c);
        this.num = num;
    }
    ...
    public void onBindViewHolder(CYCBaseViewHolder holder, int position) {
        ...
        RequestOptions requestOptions = RequestOptions.centerCropTransform().diskCacheStrategy(DiskCacheStrategy
                .AUTOMATIC);
        Glide.with(context).load(mData.getTouch_icon()).apply(requestOptions).into(iv);
    }
}
